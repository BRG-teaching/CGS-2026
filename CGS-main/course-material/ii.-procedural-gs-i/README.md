# B. Procedural GS

{% content-ref url="lecture-2.md" %}
[lecture-2.md](lecture-2.md)
{% endcontent-ref %}

{% content-ref url="tutorial-2.md" %}
[tutorial-2.md](tutorial-2.md)
{% endcontent-ref %}

{% content-ref url="exercise-2.md" %}
[exercise-2.md](exercise-2.md)
{% endcontent-ref %}

{% content-ref url="tutorial-3.md" %}
[tutorial-3.md](tutorial-3.md)
{% endcontent-ref %}

{% content-ref url="exercise-3.md" %}
[exercise-3.md](exercise-3.md)
{% endcontent-ref %}

## Files <a href="#files" id="files"></a>

In this module, you will use the following files:



{% file src="../../.gitbook/assets/CGS_EX_B-1.docx" %}

{% file src="../../.gitbook/assets/CGS_B.3dm" %}

{% file src="../../.gitbook/assets/CGS_B_tutorial_1.gh" %}

{% file src="../../.gitbook/assets/CGS_EX_B-2.docx" %}

{% file src="../../.gitbook/assets/CGS_B_tutorial_2.gh" %}
