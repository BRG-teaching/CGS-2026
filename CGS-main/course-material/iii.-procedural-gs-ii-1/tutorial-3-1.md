...
<figure><img src="../../.gitbook/assets/drawings_3_no-titles (1).jpg" alt=""><figcaption></figcaption></figure>
...
* Finally, we will intersect the first and last auxiliary line in the form diagram to find the point p\_R across which the vertical line of action of the resultant goes through. And we will move it (p\_R\_m) in order to have the freedom to change the rise of the funicular in the form diagram.&#x20;
...
* **p1** (any point on the bridge deck)
* **p\_right** (point/right cliff)
* **O1** (random pole to find resultant)
* **Ll\_LOA** (List of lines/lines of action)
* **Lal\_force** (List of auxiliary lines/force diagram)
* **y** (value of the y component/selecting anchor point of the resultant)

Output:

* **p\_R\_m** (point/resultant/moved)
{% endhint %}
...
v=[0,y,0]
p_R=rs.AddPoint([ip[0],rs.PointCoordinates(p1)[1],0])
p_R_m=rs.MoveObject(p_R,v)
...
* Then, we will create two lines in the form diagram connecting the supports with the chosen anchor of the resultant (p\_R\_m).
...
l1=rs.AddLine(p_R_m,sp_left)
l2=rs.AddLine(p_R_m,sp_right)

#building reaction forces in force diagram
v1=rs.VectorCreate(Lp_loadline[-1],p_R_m)
l1_m=rs.MoveObject(l1,v1)

v2=rs.VectorCreate(Lp_loadline[0],p_R_m)
